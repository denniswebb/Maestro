...
export function QuickActionsModal(props: QuickActionsModalProps) {
  ...
  const mainActions: QuickAction[] = [
    ...
    ...(isAiMode && activeSession?.aiTabs && activeSession.aiTabs.length > 0 && onCloseAllTabs ? [{
      ...
      action: () => { onCloseAllTabs(); setQuickActionOpen(false); }
    }] : []),
    ...(isAiMode && activeSession?.aiTabs && activeSession.aiTabs.length > 1 && onCloseOtherTabs ? [{
      ...
      action: () => { onCloseOtherTabs(); setQuickActionOpen(false); }
    }] : []),
    ...(isAiMode && activeSession?.aiTabs && (() => {
      const activeTabIndex = activeSession.aiTabs.findIndex(t => t.id === activeSession.activeTabId);
      return activeTabIndex > 0 && onCloseTabsLeft;
    })() ? [{
      id: 'closeTabsLeft',
      label: 'Close Tabs to Left',
      shortcut: tabShortcuts?.closeTabsLeft,
      action: () => { onCloseTabsLeft?.(); setQuickActionOpen(false); }
    }] : []),
    ...(isAiMode && activeSession?.aiTabs && (() => {
      const activeTabIndex = activeSession.aiTabs.findIndex(t => t.id === activeSession.activeTabId);
      return activeTabIndex < activeSession.aiTabs.length - 1 && onCloseTabsRight;
    })() ? [{
      id: 'closeTabsRight',
      label: 'Close Tabs to Right',
      shortcut: tabShortcuts?.closeTabsRight,
      action: () => { onCloseTabsRight?.(); setQuickActionOpen(false); }
    }] : []),
